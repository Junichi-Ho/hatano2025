...
def create_dataframe_from():
    ...
    # 全角数字を半角数字に変換するマッピングを作成
    fullwidth_to_halfwidth = str.maketrans('０１２３４５６７８９', '0123456789')

    # "T"列のデータを補正
    combined_df["T"] = combined_df["T"].apply(lambda x: x.translate(fullwidth_to_halfwidth) if isinstance(x, str) else x)  # 全角数字を半角に変換
    combined_df["T"] = combined_df["T"].str.upper()  # 小文字を大文字に変換
    combined_df["T"] = combined_df["T"].replace({'DR': '1W'})  # "DR"を"1W"に変換
    #combined_df["T"] = combined_df["T"].apply(lambda x: x[1] + x[0] if isinstance(x, str) and len(x) == 2 and x[0].isdigit() else x)  # 数字+アルファベットの形式に変換
    #combined_df["T"] = combined_df["T"].apply(lambda x: x if not isinstance(x, str) or len(x) != 2 or not x[1].isalpha() else x[1] + x[0])  # 数字+アルファベットの形式に変換
    combined_df["T"] = combined_df["T"].apply(lambda x: x if not isinstance(x, str) or len(x) != 2 or not x[0].isalpha() else x[1] + x[0])  # 数字+アルファベットの形式に変換
...
def filter_for_drilldown(combined_df):
    # 年でFilterするオプション#Streamlitのマルチセレクト
    year_list = list(combined_df["y"].unique())
    default_years = ["24", "23", "22", "21", "20"]  # デフォルトで選択する年
    select_year = st.multiselect("年でFilterling", year_list, default=[year for year in default_years if year in year_list])
    combined_df = combined_df[(combined_df["y"].isin(select_year))]
    
    #hole_list = list(combined_df["hole"].unique())
    #select_hole = st.selectbox("HoleでFilterling", hole_list, index=hole_list.index(1))
    #combined_df = combined_df[(combined_df["hole"] == select_hole)]

    # 集計対象のホール番号を指定
    target_holes = [1, 3, 5, 6, 9, 12, 13]

    # 各ホールごとに集計して表示
    for hole in target_holes:
        #st.write(f"Hole {hole} の集計結果:")

        # 指定したホールでフィルタリング
        hole_df = combined_df[combined_df["hole"] == hole]

        # "T"の種類と個数を表示
        t_counts = hole_df["T"].value_counts()

        # "TR"が"ROB"または"LOB"の個数をカウント
        ob_counts = hole_df[hole_df["TR"].isin(["ROB", "LOB"])]["T"].value_counts()

        # "T"の種類と"OB数"を結合したデータフレームを作成
        result_df = pd.DataFrame({'shots': t_counts, 'OB数': ob_counts}).fillna(0).astype(int)

        # (shots-OB数)/shotsを計算し、パーセンテージとして新しい列を追加
        result_df['OB回避率'] = ((result_df['shots'] - result_df['OB数']) / result_df['shots'] * 100).round(1)

        # "T"ごとのScoreの平均を計算し、小数点2位まで表示
        score_mean = hole_df.groupby("T")["Score"].mean().round(2)
        result_df['Score平均'] = score_mean

        # Holeの平均値を算出し、平均+1、小数点繰り上げの値を求める
        hole_mean = hole_df["Score"].mean()
        XAve = math.ceil(hole_mean + 1)
        Xma = math.floor(hole_mean) + 1  # 小数点を切り捨て

        st.write(f"Hole {hole} の集計結果: XAve={XAve}, Xma={Xma}")

        # ScoreでXAve以上はXAveにして、それぞれTの種類ごと平均値を求める
        hole_df["AdjustedScore"] = hole_df["Score"].apply(lambda x: min(x, XAve))
        xaverage_mean = hole_df.groupby("T")["AdjustedScore"].mean().round(2)
        result_df['XAverage'] = xaverage_mean

        # ScoreでXma以上はXmaにして、それぞれTの種類ごと平均値を求める
        hole_df["AdjustedScoreXma"] = hole_df["Score"].apply(lambda x: min(x, Xma))
        xmatch_mean = hole_df.groupby("T")["AdjustedScoreXma"].mean().round(2)
        result_df['XMatch'] = xmatch_mean

        # shotsの多い4つまでを選択し、XAverageを昇順で表示
        top4_df = result_df.nlargest(4, 'shots').sort_values(by='XAverage')

        # OB回避率を小数点1桁にフォーマット
        top4_df['OB回避率'] = top4_df['OB回避率'].map('{:.1f}'.format)

        # Score平均とXAverageを小数点2桁にフォーマット
        top4_df['Score平均'] = top4_df['Score平均'].map('{:.2f}'.format)
        top4_df['XAverage'] = top4_df['XAverage'].map('{:.2f}'.format)
        top4_df['XMatch'] = top4_df['XMatch'].map('{:.2f}'.format)
...
def main():
    ...
    # PPの選択
    pp_list = list(combined_df["PP"].unique())
    select_pp = st.selectbox("Pin PositionでFilterling", pp_list, index=pp_list.index(3))

    # 選択されたHoleとPPでデータをフィルタリング
    filtered_df = combined_df[(combined_df["hole"] == select_hole) & (combined_df["PP"] == select_pp)]

    # フィルタリングされたデータを表示
    st.dataframe(filtered_df)

    #filter_for_drilldown(combined_df)
    #SideBar表示
